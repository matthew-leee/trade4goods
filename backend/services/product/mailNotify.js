module.exports = class {
    ...
    async sendOfferNotification(product_id) {
        try {
            let product = await this.knex('product').where('product_id', product_id)
            product = product[0]
            ...
            const mailOptions = {
                ...
                html: `<p>Heads Up!Your product ${product_name} has been offered<p>` +
                    `<p>Visit <a>https://${process.env.FRONTEND_URL}/userPage</a>to view your offer</p>` +
    ...
    async sendAcceptOfferNotification(product_id) {
        try {
            ...
            const mailOptions = {
                ...
                html: `<p>Heads Up!Your offer on roduct ${product_name} has been accept<p>` +
                    `<p>Visit <a>https://${process.env.FRONTEND_URL}</a>to manage the trade</p>` +
                    `<p>This is an automatic message sent by system, please do not reply to this message</p>`// html body
            };
            this.transporter.sendMail(mailOptions)
        } catch (err) {
