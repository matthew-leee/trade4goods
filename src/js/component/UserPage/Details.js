...
const mapDispatchToProps = (dispatch, ownProps) => {
    return {
        ...
        handleSubmitComment: async (id, comment, allUsers) => {
            try {
                const res = await Axios("https://localhost:8080/api/comment", {
                    method: "post",
                    data: { product_id: id, comment: comment },
                    withCredentials: true
                })
                if (res.status == 201) {
                    const products = await Axios('https://localhost:8080/api/allProducts/', {
                        method: "get",
                        withCredentials: true
                    })
                    ...
                    const comments = ids.map(async (comment_id) => {
                        const sth = await Axios(`https://localhost:8080/api/comment/${comment_id}`, {
                            method: "get",
                            withCredentials: true
                        })
